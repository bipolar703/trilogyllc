...
const ServiceDetail = () => {
  ...
  const serviceDetails = t(`services.items.${localeKey}`, { returnObjects: true }) as {
    title: string;
    description: string;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className={`relative overflow-hidden ${styles.headerBackground}`}>
        ...
        <div className={`${styles.light1} bg-brand-gold/20 w-96 h-96 -top-24 -left-24`} />
        <div className={`${styles.light2} bg-brand-bronze/20 w-96 h-96 -bottom-24 -right-24`} />
        <div className={`${styles.light3} bg-white/10 w-64 h-64 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2`} />

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-32">
          <div className="text-center">
            <h1 className={`text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 ${styles.gradientText}`}>
              {serviceDetails.title}
            </h1>
            <p className={`text-xl md:text-2xl text-gray-300 max-w-3xl mx-auto ${styles.subtleText}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="bg-white rounded-2xl shadow-lg p-8 md:p-12">
          ...
          {serviceDetails.sections?.overview && (
            <div className="mb-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                {Object.entries(serviceDetails.sections.overview.highlights).map(([key, value]) => (
          ...
          {serviceDetails.sections?.methodology && (
            <div className="mb-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {serviceDetails.sections.methodology.steps.map((step, index) => (
          ...
          {(serviceDetails.sections?.solutions || serviceDetails.sections?.documentation) && (
            <div className="mb-16">
              <h2 className="text-3xl font-bold text-gray-900 mb-8">
                {serviceDetails.sections?.solutions?.title || serviceDetails.sections?.documentation?.title}
              </h2>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {(serviceDetails.sections?.solutions?.categories || serviceDetails.sections?.documentation?.categories)?.map((category, index) => (
                  <div key={index} className="bg-gray-50 rounded-xl p-6">
                    <h3 className="text-xl font-semibold text-gray-900 mb-4">
                      {category.title}
                    </h3>
                    <ul className="space-y-3">
                      {(category.features || category.items)?.map((item, itemIndex) => (
